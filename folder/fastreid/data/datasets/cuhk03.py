...
class CUHK03(ImageDataset):
    ...
    FileNotFoundError: [Errno 2] No such file or directory: './data\\cuhk03\\images_labeled\\1_478_1_05.png'
    """
    dataset_dir = 'cuhk03'
    dataset_url = None
    dataset_name = "cuhk03"
    ...
    def __init__(self, root='datasets', split_id=0, cuhk03_labeled=True, cuhk03_classic_split=False, **kwargs):
        ...
        for img_path, pid, camid in train:
            # new_pid = f"{self.dataset_name}_{pid}"
            # new_camid = f"{self.dataset_name}_{camid}"
            #./data\\cuhk03\\images_labeled\\1_478_1_05.png
            lst = img_path.split('\\')
            img_path = osp.join('/home/phongnn/test/test/fast-reid/datasets/cuhk03', lst[2], lst[3])
            tmp_train.append((img_path, pid, camid))
        train = tmp_train
        print(train[0])
        del tmp_train
        query = split['query']
