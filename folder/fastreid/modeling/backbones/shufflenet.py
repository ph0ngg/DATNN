...
class ShuffleNetV2(nn.Module):
    ...
    def __init__(self, bn_norm, model_size='0.5x'):
        ...
        #     nn.ReLU(inplace=True),
        # )
        self.view_predictor1 = nn.Conv2d(in_channels= self.stage_out_channels[1], out_channels= 128, kernel_size= (3, 3), stride= 2, padding= 1)
    ...
    def forward(self, x):
        ...
        x1 = self.conv_last1(x)
        x2 = self.conv_last2(x)
        x3 = self.conv_last3(x)
...
def build_shufflenetv2_backbone():
    # fmt: off
    bn_norm       = "BN"
    model_size    = "0.5x"
    pretrain = False
