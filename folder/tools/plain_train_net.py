...
def do_train(cfg, model, resume=False):
    ...
    with EventStorage(start_iter) as storage:
        for epoch in range(start_epoch, max_epoch):
            ...
            # model.backbone.dropout.eval()
            # model.backbone.bn1.eval()
            # model.backbone.bn2.eval()

    #---------
            storage.epoch = epoch
           # print(model.backbone(img)[1])
           # print('back', model.backbone(img)[1])
            # print('side', model.backbone(img3)[1])
...
img = cv2.imread('/home/phongnn/test/test/market1501/Market-1501-v15.09.15/bounding_box_test/0626_c6s2_019818_05.jpg')
...
def main(args):
    ...
    state_dict = torch.load('/home/phongnn/test/test/fast-reid/tools/model_best.pth')
    model.load_state_dict(state_dict['model'], strict = True)
    #model.backbone.load_state_dict(state_dict, strict = False)
    # osnet_weight = torch.load('/home/phongnn/test/test/fast-reid/tools/osnet_x1_0_imagenet.pth')
    # dictt_conv5 = {}
    # for key, value in osnet_weight.items():
    #     if key.startswith('conv5'):
    #         dictt_conv5[key[6:]] = value
    # model.backbone.conv5.load_state_dict(dictt_conv5, strict = True)
